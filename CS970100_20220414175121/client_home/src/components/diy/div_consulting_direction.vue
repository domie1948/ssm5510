<template>
	<div class="diy_details_box diy_div_consulting_direction">
		<div class="warp">
			<div class="container">
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					...
					source_field: "consulting_direction_id",
					source_id: this.obj["consulting_direction_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/consulting_direction/set?consulting_direction_id=' + _this.obj["consulting_direction_id"], {
								praise_len
							}, (res) => {
								if(res.result){
