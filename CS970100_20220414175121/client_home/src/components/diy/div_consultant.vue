<template>
	<div class="diy_details_box diy_div_consultant">
		<div class="warp">
			<div class="container">
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					...
					source_field: "consultant_id",
					source_id: this.obj["consultant_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/consultant/set?consultant_id=' + _this.obj["consultant_id"], {
								praise_len
							}, (res) => {
								if(res.result){
