		        ...
		        <div class="row">
		            <div class="col-12">
		                ...
		                <div class="diy_recommend">
		                    ...
		                    <div class="more_box">
		                        <router-link to="/consultant_information/list" class="more">
		                            <span>更多</span>
		                        </router-link>
		                    </div>
		                    <div class="switch" @click="show_list_consultant_information = !show_list_consultant_information">
		                        <div class="switch_box">
		                            <span v-if="show_list_consultant_information"> 切换表格 </span>
		                            <span v-else> 切换列表 </span>
		                        </div>
		                    </div>
		                </div>
		                <!--咨询师信息推荐列表组件 -->
		                <list_consultant_information v-if="show_list_consultant_information" :list="list_consultant_information" />
		                <div class="overflow-auto" v-else>
		                    <table id="list_diy" role="table" aria-busy="false"
		                           :aria-colcount="fields_consultant_information.length"
		                           class="table b-table table-striped table-hover">
		                        <thead>
		                        <tr>
		                            <th v-for="(o,i) in fields_consultant_information" :key="i">
		                                {{o.label}}
		                            </th>
		                        </tr>
		                        </thead>
		                        <tbody>
		                        <tr v-for="(o, i) in list_table_consultant_information" :key="i" @click="to_details('consultant_information',o,'consultant_information_id')">
		                            <td v-for="(oj,n) in fields_consultant_information">
		                                <img v-if="oj.type && oj.type == '图片' " :src="$fullUrl(o[oj.key])" alt=""
		                                     v-default-img="'/img/default.png'">
		        ...
		        <div class="row">
		            <div class="col-12">
		                ...
		                <div class="diy_recommend">
		                    ...
		                    <div class="more_box">
		                        <router-link to="/decompression_base_camp/list" class="more">
		                            <span>更多</span>
		                        </router-link>
		                    </div>
		                    <div class="switch" @click="show_list_decompression_base_camp = !show_list_decompression_base_camp">
		                        <div class="switch_box">
		                            <span v-if="show_list_decompression_base_camp"> 切换表格 </span>
		                            <span v-else> 切换列表 </span>
		                        </div>
		                    </div>
		                </div>
		                <!--减压大本营推荐列表组件 -->
		                <list_decompression_base_camp v-if="show_list_decompression_base_camp" :list="list_decompression_base_camp" />
		                <div class="overflow-auto" v-else>
		                    <table id="list_diy" role="table" aria-busy="false"
		                           :aria-colcount="fields_decompression_base_camp.length"
		                           class="table b-table table-striped table-hover">
		                        <thead>
		                        <tr>
		                            <th v-for="(o,i) in fields_decompression_base_camp" :key="i">
		                                {{o.label}}
		                            </th>
		                        </tr>
		                        </thead>
		                        <tbody>
		                        <tr v-for="(o, i) in list_table_decompression_base_camp" :key="i" @click="to_details('decompression_base_camp',o,'decompression_base_camp_id')">
		                            <td v-for="(oj,n) in fields_decompression_base_camp">
		                                <img v-if="oj.type && oj.type == '图片' " :src="$fullUrl(o[oj.key])" alt=""
		                                     v-default-img="'/img/default.png'">
										<span v-else-if="oj.type && oj.type == 'UID' ">
											<span style="display: none">
...
<script>
	...
	import list_decompression_base_camp from "@/components/diy/list_decompression_base_camp.vue";
	...
	export default {
		...
		data() {
			return {
				...
				show_list_consultant_information: true,
				list_consultant_information: [],
				vm_arr_consultant_information: [
				...
				fields_consultant_information: [
					...
					{
						key: "consultant_gender",
						label: "咨询师性别",
						type: "文本"
					},
					{
						key: "consulting_direction",
						label: "咨询方向",
						type: "下寻"
					},
					{
						key: "consultant_avatar",
						label: "咨询师头像",
						type: "图片"
					},
					{
				...
				show_list_decompression_base_camp: true,
				list_decompression_base_camp: [],
				vm_arr_decompression_base_camp: [
					"decompression_base_camp_id",
					"decompression_title",
					"consultant_name",
					"decompression_cover",
					"consultant_user",
					"decompression_audio",
					"decompression_content",
				],
				fields_decompression_base_camp: [
					{
						key: "decompression_title",
						label: "减压标题",
						type: "文本"
					},
					{
		...
		methods: {
			...
			get_consultant_information() {
				let url = "~/api/consultant_information/get_list?";
				this.$get(url, {
					"page": 1,
					"size": 8
				}, (json) => {
					if (json.result) {
						this.list_consultant_information = json.result.list;
					}
				})
			},
			// 获取减压大本营列表
			get_decompression_base_camp() {
				let url = "~/api/decompression_base_camp/get_list?";
				this.$get(url, {
					"page": 1,
					"size": 8
				}, (json) => {
					if (json.result) {
						this.list_decompression_base_camp = json.result.list;
					}
				})
			},
		...
		mounted() {
			this.get_consultant_information();
			this.get_decompression_base_camp();
			this.get_menu();
			this.get_slides();
			this.get_article();
			this.get_link();
		...
		computed: {
		    list_table_consultant_information() {
		        var list = this.list_consultant_information;
		        var list_table = [];
		        for (let i = 0; i < list.length; i++) {
		            list_table[i] = {};
		            this.vm_arr_consultant_information.map((o) => {
		                // 第二个中括号是对象的属性
		    ...
		    list_table_decompression_base_camp() {
		        var list = this.list_decompression_base_camp;
		        var list_table = [];
		        for (let i = 0; i < list.length; i++) {
		            list_table[i] = {};
		            this.vm_arr_decompression_base_camp.map((o) => {
		                // 第二个中括号是对象的属性
		                list_table[i][o] = list[i][o] || "";
		            });
		        }
