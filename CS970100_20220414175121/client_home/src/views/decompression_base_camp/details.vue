<template>
	<div class="diy_details page_decompression_base_camp" id="decompression_base_camp_details">
		<div class='warp'>
			<div class='container'>
				<div class='row'>
					<div class='col'>
						<div class='card_decompression_base_camp'>
							<div_decompression_base_camp :obj="obj"></div_decompression_base_camp>
						</div>
					</div>
				</div>
			</div>
...
<script>
	import div_decompression_base_camp from "@/components/diy/div_decompression_base_camp.vue";
	...
	export default {
		...
		data(){
			return{
				...
				form_comment: {
					...
					source_field: "decompression_base_camp_id",
					source_id: 0,
					reply_to_id: 0,
				},
				count_pages: 1, // 页面数
		...
		methods:{
			...
			add_hits(obj) {
				obj["hits"] = obj["hits"] + 1;
				var hits = obj["hits"];
				this.$post('~/api/decompression_base_camp/set?decompression_base_camp_id=' + obj["decompression_base_camp_id"], {
					hits
				}, (res) => {
					if(res.result){
						...
						var body = {
							source_table: "decompression_base_camp",
							source_field: "decompression_base_camp_id",
							source_id: this.obj.decompression_base_camp_id,
			...
			get_obj_after(json) {
				...
				if (json && json.result && json.result.obj) {
					...
					this.form_comment.source_id = this.obj.decompression_base_camp_id;
				}
			},
			/**
			 * 初始化收藏状态
			...
			get_collect(obj) {
				...
				var query = {
					source_table: "decompression_base_camp",
					source_field: "decompression_base_camp_id",
					source_id: obj["decompression_base_camp_id"],
					user_id
				};
		 ...
		 /****
			...
			change_collect() {
				...
				if (this.state_collect) {
					...
					query.title = this.obj.decompression_title
					query.img = this.obj.decompression_cover
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/decompression_base_camp/set?decompression_base_camp_id=' + _this.obj["decompression_base_camp_id"], {
								praise_len
							}, (res) => {
								if(res.result){
