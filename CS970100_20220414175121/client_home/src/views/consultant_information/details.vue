<template>
	<div class="diy_details page_consultant_information" id="consultant_information_details">
		<div class='warp'>
			<div class='container'>
				<div class='row'>
					<div class='col'>
						<div class='card_consultant_information'>
							<div_consultant_information :obj="obj"></div_consultant_information>
						</div>
					</div>
				</div>
			</div>
...
<script>
	import div_consultant_information from "@/components/diy/div_consultant_information.vue";
	...
	export default {
		...
		data(){
			return{
				...
				form_comment: {
					...
					source_field: "consultant_information_id",
					source_id: 0,
					reply_to_id: 0,
				},
				count_pages: 1, // 页面数
		...
		methods:{
			...
			add_hits(obj) {
				obj["hits"] = obj["hits"] + 1;
				var hits = obj["hits"];
				this.$post('~/api/consultant_information/set?consultant_information_id=' + obj["consultant_information_id"], {
					hits
				}, (res) => {
					if(res.result){
						...
						var body = {
							source_table: "consultant_information",
							source_field: "consultant_information_id",
							source_id: this.obj.consultant_information_id,
			...
			get_obj_after(json) {
				...
				if (json && json.result && json.result.obj) {
					...
					this.form_comment.source_id = this.obj.consultant_information_id;
				}
			},
			/**
			 * 初始化收藏状态
			...
			get_collect(obj) {
				...
				var query = {
					source_table: "consultant_information",
					source_field: "consultant_information_id",
					source_id: obj["consultant_information_id"],
					user_id
				};
		 ...
		 /****
			...
			change_collect() {
				...
				if (this.state_collect) {
					...
					query.title = this.obj.consultant_name
					query.img = this.obj.consultant_avatar
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/consultant_information/set?consultant_information_id=' + _this.obj["consultant_information_id"], {
								praise_len
							}, (res) => {
								if(res.result){
