		...
		<el-form ref="form" :model="form" status-icon label-width="120px" v-if="is_view()">
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','consultant_name') || $check_field('add','consultant_name') || $check_field('set','consultant_name')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="咨询师姓名" prop="consultant_name">
					<el-input id="consultant_name" v-model="form_sub['consultant_name']" placeholder="请输入咨询师姓名"
							  v-if="user_group === '管理员' || (form_sub['consultant_id'] && $check_field('set','consultant_name')) || (!form_sub['consultant_id'] && $check_field('add','consultant_name'))"></el-input>
					<div v-else-if="$check_field('get','consultant_name')">{{form_sub['consultant_name']}}</div>
				</el-form-item>
			</el-col>
			<el-col v-if="user_group === '管理员' || $check_field('get','consultant_gender') || $check_field('add','consultant_gender') || $check_field('set','consultant_gender')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="咨询师性别" prop="consultant_gender">
					<el-select id="consultant_gender" v-model="form_sub['consultant_gender']"
						v-if="user_group === '管理员' || (form_sub['consultant_id'] && $check_field('set','consultant_gender')) || (!form_sub['consultant_id'] && $check_field('add','consultant_gender'))">
						<el-option v-for="o in list_consultant_gender" :key="o" :label="o" :value="o">
						</el-option>
					</el-select>
					<div v-else-if="$check_field('get','consultant_gender')">{{form_sub['consultant_gender']}}</div>
				</el-form-item>
			</el-col>
...
<script>
	...
	export default {
        ...
        form_sub = this.events("submit_before", Object.assign({}, form_sub)) || form_sub;
				if(form_sub["consultant_id"]){
					// 提交事件
					this.$post("~/api/" + this.group_table + "/set?consultant_id=" + form_sub["consultant_id"],form_sub,(res)=>{
			...
			is_view(){
				...
				if(!bl){
					bl = this.$check_action('/consultant/table','add');
					console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
				}
				if(!bl){
					bl = this.$check_action('/consultant/table','set');
					console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
				}
				if(!bl){
					bl = this.$check_action('/consultant/view','add');
					console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
				}
				if(!bl){
					bl = this.$check_action('/consultant/view','set');
					console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
				}
				if(!bl){
					bl = this.$check_action('/consultant/view','get');
